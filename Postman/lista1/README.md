...
3. Selecione os arquivos **`Lista1.postman_collection.json`** e **`Lista1.postman_environment.json`** para importar a coleção e o ambiente.
...
1. **Navegue até o diretório** onde os arquivos **`Lista1.postman_collection.json`** e **`Lista1.postman_environment.json`** estão localizados:
   ```sh
...
2. **Execute o comando Newman** para rodar a coleção de testes e gerar um relatório em HTML:
    ```sh
   newman run Lista1.postman_collection.json -e Lista1.postman_environment.json -r htmlextra
   ```
